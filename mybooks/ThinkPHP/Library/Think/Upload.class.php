...
class Upload
{
    ...
    public function upload($files = '')
    {
        ...
        foreach ($files as $key => $file) {
            ...
            /* 获取文件hash */
            if ($this->hash) {
                $file['md5']  = md5(time());//md5_file($file['tmp_name']);
                $file['sha1'] = md5(time());//sha1_file($file['tmp_name']);
            }
            ...
            if (in_array($ext, array('gif', 'jpg', 'jpeg', 'bmp', 'png', 'swf','mkv'))) {
    ...
    public function upload2($files = '')
    {
        ...
        foreach ($files as $key => $file) {
            ...
            /* 对图像文件进行严格检测 */
            $ext = strtolower($file['ext']);
            if (!in_array($ext, array('wmv','mov','mpg','ts','3gp','mp4','wma','mp3'))) {
                $this->error = '只能上传视频!';
                continue;
            }
